...
@push('scripts')
    <script>
        $(document).ready(function(){
            ...
            var tableReklame = $('.table-datatable').DataTable({
                responsive: true,
                language: {
                    search: "",
                ...
                serverSide: true,
                ajax: {
                    url: '{{ url('api/reklame-datatable') }}',
                    data: function(d){
                        d.reklame_klasifikasi_id = $('#select-reklame-klasifikasi').val();
            ...
            $('#select-reklame-klasifikasi').on('select2:select', function(){
                tableReklame.ajax.reload();
            });
            ...
            $('.btnExport').on('click', function(e){
                ...
                var reklame_klasifikasi = $('#select-reklame-klasifikasi').val();
                var tahun = $('#select-tahun').val();
                var kecamatan = $('#select-kecamatan').val();
                var statusVerifikasi = $('[name="radio-verifikasi"]:checked').val();

                window.open('{{ url('reklame/export') }}?tahun=' + tahun
                    + (reklame_klasifikasi ? '&reklame_klasifikasi_id=' + reklame_klasifikasi : '')
            ...
            <div class="row">
                ...
                <div class="col-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">Potensi Pajak Reklame</li>
                    </ol>
                </div>
            </div>
        </div>
    ...
    <div class="container-fluid">
        <div class="row project-cards">
            <div class="col-md-12 project-list">
                <div class="card">
                    <div class="row">
                        @canany(['manage-reklame', 'admin'])
                        <div class="col-auto">
                            <a class="btn btn-light" href="javascript:void(0);" data-bs-toggle="modal" data-original-title="Import Data Reklame" data-bs-target="#import-modal">
                                <i data-feather="file-plus"></i> Import
                            </a>
                        </div>
                        @endcanany
                        <div class="col-auto">
                            <a class="btn btn-light btnExport" href="{{ url('reklame/export') }}">
                        ...
                        <div class="col-auto" style="display:none">
                            <div class="dropdown2">
                                ...
                                <div class="dropdown-content2">
                                    <a class="dropdown-item btnPrint" href="{{ url('reklame/print-potensi-pajak') }}">Potensi Pajak</a>
                                    <a class="dropdown-item btnPrint" href="{{ url('reklame/print-tingkat-hunian') }}">Tingkat Hunian</a>
                                </div>
                            </div>
                        </div>
                        @canany(['manage-reklame', 'admin'])
                        <div class="col-auto ms-auto">
                            <a class="btn btn-success" href="{{ url('/reklame/create') }}">
                                <i data-feather="plus-square"></i> Tambah Reklame
                            </a>
                        </div>
                        @endcanany
                    </div>
            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                        <h5 class="mb-3">Daftar Reklame</h5>
                        <span>Berikut ini adalah daftar potensi pajak <b>Reklame</b>.</span>
                    </div>
                    <div class="card-body">
                        ...
                        <div class="row">
                            ...
                            <div class="col-md-4">
                                <div class="mb-3">
                                    <label class="col-form-label">Filter Klasifikasi Reklame</label>
                                    <select class="select2-single" id="select-reklame-klasifikasi">
                                        <option value="">SEMUA</option>
                                        @foreach($klasifikasis as $row)
                                            <option value="{{ $row->id }}">{{ $row->reklame_klasifikasi_deskripsi }}</option>
                                        @endforeach
                                    </select>
                                </div>
                            </div>
    ...
    <div class="modal fade" id="import-modal" tabindex="-1" role="dialog" aria-labelledby="import-modal-label" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <form method="post" enctype="multipart/form-data" action="{{ url('reklame/import') }}">
                    @csrf
                    <div class="modal-header">
                        <h5 class="modal-title" id="import-modal-label">Import Data Reklame</h5>
                    ...
                    <div class="modal-body">
                        <div class="mb-3">
                            ...
                            <div class="row mt-2 mb-3">
                                <div class="col col-auto">
                                    <label class="mb-0 radio-input" for="radio_reklame">
                                        <input class="radio_animated" id="radio_reklame" type="radio" name="import_tipe" value="Reklame" checked> <span>Import Data Reklame</span>
                                    </label>
                                </div>
                            </div>
                        </div>
                        ...
                        <div class="alert alert-light dark" role="alert">
                            <p>Download file format import <span class="import-type">Reklame</span>: <a class="form-download" href="{{ asset('downloads/format/reklame.xls') }}" download>disini</a>.</p>
                        </div>
                    </div>
