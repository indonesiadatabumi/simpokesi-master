...
<head>
    ...
    <title>Rekapitulasi Potensi Pajak Hotel</title>
    <style>
        * {
                ...
                <tr>
                    @if($keyKamar === 0)
                        ...
                        <td rowspan="{{ $row->jenis_kamar()->count() }}">
                            ...
                            {{ @$row->hotel_pemilik ?? '-' }} <br>
                            Persentase: {{ @$row->hotel_persentase_pajak }}%
                        </td>
                        <td rowspan="{{ $row->jenis_kamar()->count() }}">
                    ...
                    <td class="text-right">{{ generateRupiah($rowKamar->hotel_jenis_kamar_tarif, false) }}</td>
                    <td class="text-right">{{ generateRupiah($rowKamar->hotel_jenis_kamar_potensi_pajak, false) }}</td>
                </tr>
                @endforeach

                <tr>
                    <th class="text-center first-col" colspan="7">Sub Total</th>
                    <th class="text-right">{{ generateRupiah($row->jenis_kamar()->get()->sum('hotel_jenis_kamar_potensi_pajak'), false) }}</th>
                </tr>
            @endif
        @endforeach
        <tr>
            <th class="text-center first-col" colspan="7">TOTAL POTENSI PENDAPATAN PAJAK</th>
            <th class="text-right">{{ generateRupiah($hotels->sum('hotel_potensi_pajak'), false) }}</th>
        </tr>
    </table>
