...
@push('scripts')
    <script>
        $(document).ready(function(){
            ...
            var tableParkir = $('.table-datatable').DataTable({
                responsive: true,
                language: {
                    search: "",
                ...
                serverSide: true,
                ajax: {
                    url: '{{ url('api/parkir-datatable') }}',
                    data: function(d){
                        d.parkir_klasifikasi_id = $('#select-parkir-klasifikasi').val();
                ...
                columns: [
                    ...
                    { data: 'parkir_npwpd', name: 'parkir_npwpd' },
                    { data: 'parkir_nama', name: 'parkir_nama' },
                    { data: 'parkir_pemilik', name: 'parkir_pemilik' },
                    { data: 'action', name: 'action', searchable: false, width: 150 }
                ]
            });

            $('#select-parkir-klasifikasi').on('select2:select', function(){
                tableParkir.ajax.reload();
            });
            ...
            $('.btnExport').on('click', function(e){
                ...
                var parkir_klasifikasi = $('#select-parkir-klasifikasi').val();
                var tahun = $('#select-tahun').val();
                var kecamatan = $('#select-kecamatan').val();
                var statusVerifikasi = $('[name="radio-verifikasi"]:checked').val();

                window.open('{{ url('parkir/export') }}?tahun=' + tahun
                    + (parkir_klasifikasi ? '&parkir_klasifikasi_id=' + parkir_klasifikasi : '')
            ...
            <div class="row">
                ...
                <div class="col-6">
                    <ol class="breadcrumb">
                        ...
                        <li class="breadcrumb-item active">Potensi Pajak Parkir</li>
                    </ol>
                </div>
            </div>
        </div>
            ...
            <div class="col-md-12">
                <div class="card">
                    <div class="card-header">
                        <h5 class="mb-3">Daftar Parkir</h5>
                        <span>Berikut ini adalah daftar potensi pajak <b>Parkir</b>.</span>
                    </div>
                    <div class="card-body">
    ...
    <div class="modal fade" id="import-modal" tabindex="-1" role="dialog" aria-labelledby="import-modal-label" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                <form method="post" enctype="multipart/form-data" action="{{ url('parkir/import') }}">
                    @csrf
                    <div class="modal-header">
                        <h5 class="modal-title" id="import-modal-label">Import Data Parkir</h5>
                        <button class="btn-close" type="button" data-bs-dismiss="modal" aria-label="Close"></button>
                    </div>
                    <div class="modal-body">
                        <div class="alert alert-light dark" role="alert">
                            <p>Download file format import Parkir: <a href="{{ asset('downloads/format/parkir.xls') }}" download>disini</a>.</p>
                        </div>
                        <div class="mb-3">
